$(document).ready(function () {
    $.getScript(
      ...
      $(".InteractiveSemanticGraph").each(function (index) {
        if ($(".InteractiveSemanticGraph").length) {
          //check if div element(s) exist
          ...
          function createUrl(root, properties) {
            var url = `/w/api.php?action=ask&query=[[${encodeURIComponent(
              root
            )}]]`;
            var propertiesVar = "";
                    ...
                    if (
                      ...
                      id =
                        ...
                        data.query.results[root].printouts[properties[i]][j]
                          .value +
                        " " +
                        data.query.results[root].printouts[properties[i]][j].unit;
                    else
                      id =
                        data.query.results[root].printouts[properties[i]][
                          j
                        ].toString();
                    if (
                      data.query.results[root].printouts[properties[i]][j]
                    ...
                    if (isLabelSet(id) === false) {
                      if (setGroup && setColor) {
                        nodes.add({
                          ...
                          group: setGroup[0],
                          hidden: false,
                          url: data.query.results[root].printouts[properties[i]][
                            j
                          ].fullurl,
                          oncontext: true,
                        });
          ...
          network.on("click", function (params) {
            if (params.nodes[0] && params.event.srcEvent.ctrlKey) {
              ...
              if (
                nodesClicked.length == 2 &&
                nodesClicked[0] != nodesClicked[1]
              ) {
          ...
          network.on("doubleClick", function (params) {
            if (params.nodes[0]) {
              ...
              for (var i = 0; i < conManNodes.length; i++) {
                if (
                  !(
                    nodes.get(conManNodes[i]).oncontext ||
                    nodes.get(conManNodes[i]).manually
                  )
                ) {
              ...
              if (
                network.getConnectedNodes(params.nodes[0]).length > 1 &&
                onlyConManNodes == false
              ) {
                ...
                for (var i = 0; i < contextCreatedProps.length; i++) {
                  ...
                  for (var j = 0; j < nodes.getIds().length; j++) {
                    if (
                      contextCreatedProps[i] == nodes.get(nodes.getIds()[j]).group
                    ) {
                  ...
                  if (noNodesInNetwork === true) {
                    ...
                    contextCreatedProps.splice(
                      contextCreatedProps.indexOf(contextCreatedProps[i]),
                      1
                    );
                    i--;
                  }
          ...
          function setNodeVisibilityByVisiblePath(nodeId, rootNodeId) {
            ...
            connectedEdges.forEach(function (edge) {
              ...
              connectedNodes.forEach(function (connectedNode) {
                ...
                if (
                  setNodeVisibilityByVisiblePath(connectedNode.id, rootNodeId)
                ) {
          ...
          function legendFunctionality() {
            ...
            if (strategy == "strategy2") {
              ...
              options.groups[legendGroup].hidden =
                !options.groups[legendGroup].hidden; //toggle state
              if (options.groups[legendGroup].hidden)
                this.parentNode.childNodes[1].style.background = "#FFFFFF";
          ...
          network.on("oncontext", function (params) {
            ...
            if (timeDiff > 300) {
              ...
              if (
                ...
                fetch(
                  '/w/api.php?action=smwbrowse&browse=subject&params={"subject":"' +
                    encodeURIComponent(
                      nodes.get(
                        network.getNodeAt({
                          x: params.pointer.DOM.x,
                  ...
                  .then((data) => {
                    ...
                    if (selected_node.url) {
                      ...
                      li.addEventListener("click", function NewTab() {
                        window.open('/wiki/'+ selected_node.label );
                      });
                        ul.prepend(li);
                    }
                    var page_properties = data.query.data; //normal page
                    if (selected_node.id.includes("#")) {
                      //subobject
                      for (var i = 0; i < data.query.sobj.length; i++) {
                        if (
                          data.query.sobj[i].subject.endsWith(
                            selected_node.id.split("#").pop().replace(" ", "")
                          )
                        ) {
                    ...
                    $(".custom-menu li").click(function () {
                      ...
                      if (
                        ...
                        ].push(clickedProperty[0]);
                      }
  
                      if (
                        !(
                          (
                            contextCreatedProps.includes(clickedProperty[0]) ||
                            input.properties.includes(clickedProperty[0])
                          ) /*|| legendColors[clickedProperty[0]]*/
                        )
                      ) {
                        ...
                        propertyColor.addEventListener(
                          "click",
                          legendFunctionality
                        );
        ...
        console.log("here");
        return;}*/
            document.getElementById("edge-saveButton").onclick =
          ...
          async function pageExists(id) {
            await fetch(
              "/w/api.php?action=parse&page=" + id + "&prop=wikitext&format=json"
            )
              .then((response) => response.json())
              .then((data) => {
                if (data.error) {
          ...
          async function editWikiText(node) {
            await fetch(
              ...
              .then((data) => {
                ...
                semantic = "";
                if (
                  wikiText.search(/(\{\{Semantic\/[^}]*[\r\n]*\}[\r\n]*\})/g) >= 0
                ) {
                  //var edgeStringFound = wikiText.search(re) >= 0;
                  const found = wikiText.match(
                    /(\{\{Semantic\/[^}]*[\r\n]*\}[\r\n]*\})/g
                  );
          ...
          async function saveEdgeData(data, callback) {
            ...
            if (
              !(
                contextCreatedProps.includes(data.label) ||
                input.properties.includes(data.label)
              )
            ) {
                  ...
                  .then((data) => {
                    ...
                    if (edgeStringFound) {
                      if (editDeletedEdges["" + edgeToNode]) {
                        var newWikiText = editDeletedEdges[
                          "" + edgeToNode
                        ].replace(re, "");
                    ...
                    if (editNodes["" + edgeToNode]) {
                      var newWikiText = editNodes["" + edgeToNode].replace(
                        re,
                        ""
                      );
                ...
                await fetch(
                  ...
                  .then((data) => {
                    ...
                    if (newNodes["" + edgeFromNode]) {
                      var newWikiText = newNodes["" + edgeFromNode].replace(
                        re,
                        ""
                      );
                    ...
                    if (editNodes["" + edgeFromNode]) {
                      var newWikiText = editNodes["" + edgeFromNode].replace(
                        re,
                        ""
                      );
          ...
          function dragElement(elmnt) {
            ...
            if (document.getElementById(elmnt.id)) {
              ...
              document.getElementById("node-operation").onmousedown =
                dragMouseDown;
              document.getElementById("edge-operation").onmousedown =
                dragMouseDown;
            } else {
