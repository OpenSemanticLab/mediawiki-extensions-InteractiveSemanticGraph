isg.Data = class {
    ...
    fetchData(root, properties, nodeID, colors) {
        ...
        Promise.allSettled(promises).then(([result]) => {deferred.resolve(result);});
			...
			if (uri && this.config.uri_color_map) {
				for (const key in this.config.uri_color_map) {
					if (uri.startsWith(key)) nodeColor = this.config.uri_color_map[key];
                        ...
                        if (this.nodeExists(id) === false) { //test if node with id exists
                            this.nodes.add({
                                id: id,
                                label: label,
                                color: nodeColor,
    ...
    saveGraphChanges() {
        var alertString = "";
        OO.ui.confirm('Submit changes?').done((confirmed) => {
            if (confirmed) {
                var promises = [];
                var error_occured = false;
                ...
                for (const [key, value] of Object.entries(this.editDeletedNodes)) {
                    var params = {
                        action: 'delete',
                        title: '' + key,
                        format: 'json'
                ...
                Promise.allSettled(promises).then(([result]) => {
                    const _editTargetNodes = [...new Set(this.editTargetNodes)]; //remove duplicates
                    _editTargetNodes.forEach((title) => {
                        mwjson.api.purgePage(title);
                    });
                    this.editTargetNodes = [];
                    mw.notify('Saved', {
                        type: 'success'
                    });
                });
